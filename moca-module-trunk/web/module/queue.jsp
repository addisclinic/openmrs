...
<ul id="menu">
	<li class="first">
		<a href="${pageContext.request.contextPath}/module/moca/queue.form">
		  <spring:message code='moca.queue_pending_cases'/>
		</a>
	</li>
	<li>
		<a href="${pageContext.request.contextPath}/module/moca/queueDeferred.form">
		  <spring:message code='moca.queue_deferred_cases'/>
		</a>
	</li>
	<li >
		<a href="${pageContext.request.contextPath}/module/moca/queueClosed.form">
		  <spring:message code='moca.queue_closed_cases'/>
		</a>
	</li>
</ul>
...
<form action="${pageContext.request.contextPath}/module/moca/queue.form" 
    ...
    <table width="100%" class="boxHeader">
		...
		<!-- <c:set var="datechk" value="<%= request.getParameter(\"checkdate\")%>" />
		<c:if test="${datechk != 'on'}">
		<input type="checkbox" name="checkdate"  id="chkpdateid" title="Check to Search through Days and months"  />
		</c:if>
		<c:if test="${datechk == 'on'}">
		<input type="checkbox" name="checkdate"  id="chkpdateid" title="Check to Search through Days and months" checked=true/>
		</c:if> -->
        <!--  
        <td>
            <b class="boxHeader">
        ...
        <!--  
	        ...
	        <c:forEach var="daterows" items="${dateItems}">
		      ...
		      <c:choose>
				...
				<option onclick="clickdatefield('${daterows.days}','${daterows.months}');"
					selected>
					${daterows.textName}
				</option>
			    </c:when>
        ...
        <td align="left">
		 ...
		 <!-- <input type="checkbox" name="checkshowall" id="chkshowallid" title="Check to see all the records" onclick="checkshall(checkarchive)" /> Show all &nbsp&nbsp&nbsp
		<input type="checkbox" name="checkarchive" id="chkarchiveid" title="Check to see only Archived records" onclick="checkarchived(checkshowall)" />Show Archived &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp-->
		 <!-- For Retaining the Selected Archive Options. -->
			...
			<c:choose>
              ...
              <input type=submit id="subpro" name="subproname" value="Filter" 
                title="Search through Moca items" onclick="resetpagefiels()">
		      ...
		      <c:set var="hidprevname" 
		          value="<%= request.getParameter(\"hidprevname\")%>" />
		      <input type="hidden" name="hidprevname" id="hidprevid" 
		          value="${hidprevname}">
		      <c:set var="hidnextname" 
		          value="<%= request.getParameter(\"queuelimitname\")%>" />
            ...
            <!-- FIFO/LIFO ordering -->
            <td align="right">
                <b><spring:message code='moca.queue_sort_by'/> &nbsp&nbsp</b>
        ...
        <td align="left">
            ...
            <b><spring:message code='moca.queue_procedure_title_select'/></b>
        ...
        <td align="right">
            ...
            <b><spring:message code='moca.queue_procedure_title_search'/></b>
	...
	<!-- Checkall() sends chk(Checkbox array) -->		
								...
								<a href="${pageContext.request.contextPath}/moduleServlet/flashmediaviewer/complexObsServlet?obsId=${obs.obsId}&view=SHOW&viewType=DOWNLOAD">										
									<c:choose>
										<c:when test="${obs.valueText=='SOUND'}">
										    <img border="0" src="${pageContext.request.contextPath }/moduleResources/moca/audioThumbnail.png"/>
										</c:when>
										<c:when test="${obs.valueText=='VIDEO'}">
										    <img border="0" src="${pageContext.request.contextPath }/moduleResources/moca/videoThumbnail.png"/>
										</c:when>
										<c:when test="${obs.valueText=='BINARYFILE'}">
										    <img border="0" src="${pageContext.request.contextPath }/moduleResources/moca/binaryThumbnail.png"/>
										</c:when>
										<c:otherwise>
...
<b class="boxHeader">
 ...
 <!-- MOVED above
    Sort By
    <input type="radio" name="sortname" id="lifo" value="LIFO" 
        onclick="sortOrder(sortname);">Newest First &nbsp&nbsp&nbsp&nbsp 
    <input type="radio" name="sortname" id="fifo" value="FIFO" 
        onclick="sortOrder(sortname);">Oldest First&nbsp&nbsp&nbsp&nbsp
  --> 
 <!-- 
<select name="disformate">
<option selected>htmlformat</option>
<option>Jsonformat</option>
<option>Xmlformat</option>
</select>&nbsp&nbsp&nbsp&nbsp -->
<!--  MOVED ABOVE
    <c:if test="${pageno == 1}">
        <input type="submit" align="center" name="prevname" id="previd" 
        value="Prev" onclick="pageNationPrev('${queuelistcount}')" 
        disabled="disabled"/>  
    ...
    <c:if test="${pageno == count}">
        ...
        onclick="pageNationNext('${queuelistcount}')" disabled="disabled"/>
    </c:if>
    <c:if test="${pageno < count}">
        <input type="submit" name="nextname" id="nextid" value="Next" 
        onclick="pageNationNext('${queuelistcount}')">
    </c:if>
 
    <c:set var="gotopage" value="<%= request.getParameter(\"gotopagename\")%>" />
    &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
    Go To Page
    <input type=text name="gotopagename" id="gotopageid" value="${gotopage}" 
    size=3/>
    <input  type=submit value="Go" 
    onclick="return gotopage('${queuelimtset}','${queuelistcount}')"/>
   ...
   &nbsp&nbsp&nbsp&nbsp    Items Per Page
	<c:set var="queuelimtset" 
	   value="<%= request.getParameter(\"queuelimitname\")%>" />
	<input type=hidden name="queuelimitname" id="queuelimitid" 
	   value="${queuelimtset}"/>
	<select name="queueLimit">
	   <c:forEach var="i" begin="1" end="${queuelistcount}" step="1" 
	      varStatus="status">
...
-->
    ...
    <table width="100%" class="boxHeader">
        ...
        <td align="left">
            ...
            <c:set var="gotopage" value="<%= request.getParameter(\"gotopagename\")%>" />
                Go To Page
            <input type=text name="gotopagename" id="gotopageid" 
                value="${gotopage}" size=3/>
            <input  type=submit value="Go" 
            onclick="return gotopage('${queuelimtset}','${queuelistcount}')"/>
            </b>
        </td>
        ...
        <td align="center">
        <!--<input type=submit id="queuelimitsubmitid" name="queuelimitsubmitname" value="Items Per Page" title="Click here to set queue size">-->
            <b><spring:message code='moca.queue_items_per_page'/>
            <c:set var="queuelimtset" 
                value="<%= request.getParameter(\"queuelimitname\")%>" />
            <input type=hidden name="queuelimitname" id="queuelimitid" 
            value="${queuelimtset}"/>
            <select name="queueLimit" id="queuelimit"
                ...
                <c:forEach var="i" begin="1" end="${queuelistcount}" 
        ...
        <tr>
            ...
            <td>
                ...
                <c:forEach var="daterows" items="${dateItems}">
                    <c:set var="combodatee" 
                        value="<%= request.getParameter(\"daysarcname\")%>" />
                    <c:if test="${combodatee == daterows.days}" >
	                <option onclick="clickdatefieldarc('${daterows.days}','${daterows.months}');"  
	                    selected>
	                   ${daterows.textName}
	                </option>
                    </c:if>
            ...
            <td align="right">
                <input type=submit id="subdatearchive" name="subdatearchivename" 
                    value="Filter Date" onclick="resetpagefiels()" 
                    title="Click to see record beyond selected Date">
            </td>
            -->
        </tr>	
    </table>
